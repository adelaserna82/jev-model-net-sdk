...
void Header()
{
    ...
    Line("║  T Y P E D   D E C I S I O N S   / /   T E R M I N A L    ║", ConsoleColor.Cyan);
    Line("║  LABORATORIO RETRO                 .NET 10  ·  COMUNITARIO ║", ConsoleColor.DarkCyan);
    Line("╚════════════════════════════════════════════════════════════╝", ConsoleColor.Cyan);
    Line($"  {provider} · {(simulated ? "SIMULACIÓN" : "MODELO REAL")} · {(threshold is null ? "SIN UMBRAL CALIBRADO" : $"UMBRAL {threshold:P0}")}", ConsoleColor.Yellow);
}
...
void Render(DecisionReport report)
{
    ...
    var checkpoint = report.Evaluation.Extra is { } extra && extra.TryGetValue("routing", out var routing) &&
        routing.ValueKind == JsonValueKind.Object && routing.TryGetProperty("model", out var routedModel)
        ? $" | checkpoint: {routedModel.GetString()}" : "";
    Line($"  Proveedor: {report.Evaluation.Provider} | modelo: {report.Evaluation.Model}{checkpoint} | {report.ElapsedMilliseconds:F0} ms | tokens entrada/salida: {report.Evaluation.Usage.InputTokens}/{report.Evaluation.Usage.OutputTokens}", ConsoleColor.DarkGray);
}
...
async Task<DecisionReport> Evaluate(DemoCase item, CancellationToken ct)
{
    ...
    using var transport = simulated ? new HttpClient(new DemoHandler(item, provider)) : null;
    using var client = new DecisionClient(Options(), transport);
    return await ScenarioCatalog.RunAsync(client, item, provider, simulated, threshold, ct);
}
async Task Run(string command)
{
    ...
    if (command == "models")
    {
        if (provider == DecisionProvider.Laya) { Line("  El servidor oficial de Laya no ofrece catálogo de modelos."); return; }
        foreach (var model in (await client.ListJevModelsAsync(cancel.Token)).Models) Line($"  {model.Name} | {model.Description} | {model.ReleaseDate}", ConsoleColor.Green);
        return;
    }
    ...
    if (command == "cancel")
    {
        ...
        await client.EvaluateAsync(ScenarioCatalog.Build(item, provider), cancelled.Token);
        return;
    }
    // Estos comandos muestran cada tipo de pregunta de forma aislada.
    if (command is "noul" or "choice" or "score")
    {
        var request = ScenarioCatalog.Build(item, provider);
    ...
    Render(await ScenarioCatalog.RunAsync(client, item, provider, simulated, threshold, cancel.Token));
}
...
try
{
    if (!interactive)
    {
        ...
        if (command == "help") Line("support | returns | incident | batch | models | noul | choice | score | mixed | routing | cancel. Opciones: --laya, --simulate, --no-color, --menu");
    ...
    else
    {
        while (!cancel.IsCancellationRequested)
        {
            ...
            Line("  [5] MODELOS       Consultar modelos Jev");
            Line("  [6] MODO          Cambiar simulación / API real");
            Line("  [7] UMBRAL        Ajustar confianza para este proveedor");
            ...
            Line("  [P] PROVEEDOR     Cambiar Jev / Laya");
            Console.Write("\n  DECISIONS:\\LAB> ");
            ...
            try
            {
                switch (input.Trim())
                {
                    ...
                    case "p" or "P": provider = provider == DecisionProvider.Jev ? DecisionProvider.Laya : DecisionProvider.Jev; threshold = null; break;
                    ...
                    case "7":
                        Console.Write("  Umbral calibrado 0–1 (vacío para revisión humana): ");
                        var inputThreshold = Console.ReadLine();
                        if (string.IsNullOrWhiteSpace(inputThreshold)) threshold = null;
                        else if (double.TryParse(inputThreshold.Replace(',', '.'), NumberStyles.Float, CultureInfo.InvariantCulture, out var value) && double.IsFinite(value) && value >= 0 && value <= 1) threshold = value;
                    ...
                    case "8":
                        ...
                        var path = Path.GetFullPath($"artifacts/reports/decisions-{DateTime.UtcNow:yyyyMMdd-HHmmss}-{Guid.NewGuid():N}.json");
                    ...
                    case "9":
                        ...
                        Line($"  2. {provider} evalúa preguntas independientes: Choice, Noul y Score.");
                        Line("  3. C# combina resultados y umbrales; nunca ejecuta la acción.");
                        Line("  4. Usa un umbral solo después de calibrarlo para el proveedor.");
            ...
            catch (Exception e) when (e is DecisionException or ArgumentException or IOException or UnauthorizedAccessException)
...
catch (Exception e) when (e is DecisionException or ArgumentException) { Line("  ERROR: " + e.Message, ConsoleColor.Red); Environment.ExitCode = 1; }
