...
El catálogo de modelos pertenece a Jev mediante [`IJevModelCatalog`](../../src/TypedDecisions.Sdk/IJevModelCatalog.cs). Laya local usa el Dockerfile oficial fijado al commit de `v0.3.20`, con servidor, volumen persistente y prueba opcional declarados en [`compose.laya.yaml`](../../compose.laya.yaml). El SDK no ejecuta los pesos ni inicia Docker por sí mismo.
