...
Mantén nombres de código en inglés y documentación en español. Basa los cambios de contrato en documentación oficial de TypeSafe y Laya; identifica el proyecto siempre como SDK comunitario no oficial. Para probar Laya real en local, arranca `docker compose -f compose.laya.yaml up --build -d --wait` y ejecuta `docker compose -f compose.laya.yaml --profile smoke run --rm laya-smoke`.
